...
function register(api) {
    ...
    queue_1.browserQueue.configure(finalConfig);
    // Initialize enforcer
    enforcer = new enforcer_1.ProfileEnforcer(finalConfig);
    ...
    api.registerTool({
        ...
        async execute(_id, _params, context) {
            const status = queue_1.browserQueue.getStatus();
    ...
    api.registerTool({
        ...
        async execute(_id, _params, context) {
            ...
            if (!queue_1.browserQueue.hasLock(sessionId)) {
                return {
                    content: [
                        {
                            type: 'text',
            ...
            queue_1.browserQueue.release(sessionId);
    ...
    const watchdog = (0, watchdog_1.createWatchdog)(finalConfig, api.logger);
    api.registerService({
    ...
    if (api.registerHook) {
        api.registerHook('tool:before', async (toolName, params, context) => {
            ...
            try {
                const enforcedParams = enforcer.enforce(params);
            ...
            const acquired = await queue_1.browserQueue.acquire(sessionId, sessionName, isMainSession);
            if (!acquired) {
                // This session is now queued - the promise will resolve when it's their turn
                const position = queue_1.browserQueue.getQueuePosition(sessionId);
                const holder = queue_1.browserQueue.getLockHolder();
                const queueMsg = `🔒 Browser is busy. Current task: ${holder?.sessionName || 'unknown'}. ` +
            ...
            queue_1.browserQueue.updateActivity(sessionId);
            return { params };
        }, {
